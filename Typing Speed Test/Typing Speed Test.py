...
def start_game():
    ...
    statements = [
        ...
        "I know.I was planning to go to the mall today with one of my friends, but it snowed so much we couldn't go. ",
        "It's straight down this road on the left, next to the museum. It takes about ten minutes.",
    ...
    while MainGame:
        ...
        pygame.draw.rect(screen, (255, 255, 0), pygame.Rect(50, 100, 1100, 40), 3)
        sentence = text_font.render(statement, True, (255, 255, 255))
        screen.blit(sentence, (60, 110))

        pygame.draw.rect(screen, (0, 255, 0), pygame.Rect(50, 200, 1100, 40), 3)
        if start == 0:
            ...
            for i in range(len(inp_words)):
                for j in range(len(inp_words[i])):
                    if inp_words[i][j] == statement_words[i][j]:
                        ino = 0
                    elif inp_words[i][j] != statement_words[i][j]:
                        ino = 1
            if ino == 0:
                color = (0, 255, 0)
            elif ino == 1:
                for i in range(len(inp_words)):
                    for j in range(len(inp_words[i])):
                        if inp_words[i][j] == statement_words[i][j]:
                            color = (255, 0, 0)
                color = (255, 0, 0)
            inp_sentence = text_font.render(inp, True, color)
